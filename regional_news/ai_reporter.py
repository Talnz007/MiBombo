...
def parse_category(report: str) -> tuple:
    ...
    Returns (category_int, cleaned_report):
        ...
        3 = other    -> 'Other news'
    Defaults to 2 if tag missing (safe fallback for regional scraper).
    """
    ...
    if m:
        ...
        return cat, cleaned
    print("[!] No [CATEGORY] tag found in Grok response. Defaulting to category 2.")
    return 2, _CATEGORY_RE.sub('', report).strip()
