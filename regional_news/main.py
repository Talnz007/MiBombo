...
from config import TARGETS_FILE, WHATSAPP_TARGET, WHATSAPP_GLOBAL_GROUP, WHATSAPP_OTHER_GROUP
...
async def dispatch_item(item):
    ...
    if category == 2:
        target_group = WHATSAPP_TARGET        # "Regional news"
    elif category == 1:
        target_group = WHATSAPP_GLOBAL_GROUP  # "Automated news"
    else:
    ...
    try:
        ...
        mark_seen(url, category=category, source="regional_news")
...
async def scrape_twitter_to_queue(accounts, queue: asyncio.Queue):
    ...
    try:
        ...
        for item in results:
            ...
            if not claim_url(url, source="regional_news"):
